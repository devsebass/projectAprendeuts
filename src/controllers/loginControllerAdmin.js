  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(query, [data.email], (err, userdata) => {
      ...
      if (userdata.length > 0) {
        ...
        bcrypt.compare(data.password, admin.password, (err, isMatch) => {
          if (err) {
            return handleError(res, "Error al comparar contraseñas:", err);
          }

          if (isMatch) {
            setSessionVariables(req, admin);
            res.redirect("/admin/home");
          } else {
...
function setSessionVariables(req, admin) {
  req.session.loggedinAdmin = true;
  req.session.ideAdmin = admin.id;
  req.session.nameAdmin = admin.name;
  req.session.lastnameAdmin = admin.lastname;
  req.session.emailAdmin = admin.email;
  req.session.imageAdmin = admin.image;
}

function register(req, res) {
  if (req.session.loggedinAdmin !== true) {
    const title = "Registro de Admin - Aprende+";
    res.render("admin/register", { title });
  }
}
...
function user(req, res) {
  ...
  if (!data.image) {
    data.image = "image-1709424796354";
  }
        ...
        if (userdata.length > 0) {
          ...
          bcrypt.hash(data.password, 12).then((hash) => {
            ...
            req.getConnection((err, conn) => {
              ...
              conn.query("INSERT INTO admin SET ?", [data], (err) => {
                if (err) {
                  console.error(
  ...
  req.getConnection((err, conn) => {
    conn.query(
      ...
      (err, userdata) => {
        ...
        conn.query(
          "UPDATE admin SET token=?, expired=? WHERE email=?",
          [token, expired, email],
          (err, rows) => {
            if (err) {
              console.error(
        ...
        const resetUrl = `http://${req.headers.host}/admin/resetPassword/${token}`;
        sentMailPasswordAdmin(resetUrl, name, lastname, email);
...
function validationToken(req, res) {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query("SELECT * FROM admin WHERE token = ?", [token], (err) => {
      if (err) {
        console.error("Error al ejecutar la consulta:", err);
        return res.status(500).send("Error interno del servidor");
      }
    ...
    conn.query(
      ...
      (err, results) => {
        ...
        bcrypt.hash(newPassword, 12, (err, newHash) => {
          ...
          conn.query(
            "UPDATE admin SET token=?, expired=?, password=? WHERE email=?",
...
sentMailPasswordAdmin = async (token, name, lastname, email) => {
  const config = {
    host: "smtp.gmail.com",
    port: 587,
    auth: {
      ...
      <body>
          <div class="container">
              ...
              <div class="container-data">
                  <p class="text-data">Hola Admin, ${name} ${lastname}</p>
